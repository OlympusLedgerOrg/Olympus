...
if [ -z "${OLYMPUS_ADMIN_KEY:-}" ]; then
    if command -v openssl &>/dev/null; then
        export OLYMPUS_ADMIN_KEY="$(openssl rand -hex 32)"
    elif [ -r /dev/urandom ]; then
        _raw_admin_key="$(dd if=/dev/urandom bs=32 count=1 2>/dev/null | od -An -tx1 | tr -d ' \n')"
        if [ ${#_raw_admin_key} -ne 64 ]; then
            fail "Generated admin key has unexpected length (${#_raw_admin_key}). Install openssl and re-run."
        fi
        export OLYMPUS_ADMIN_KEY="$_raw_admin_key"
    else
        fail "Cannot generate a random admin key — install openssl or use /dev/urandom."
    fi
    ok "OLYMPUS_ADMIN_KEY generated for local admin user management."
    warn "Use this key in the Admin page to create users and assign access:"
    echo "   OLYMPUS_ADMIN_KEY=${OLYMPUS_ADMIN_KEY}"
else
    ok "OLYMPUS_ADMIN_KEY already set — using existing value."
fi
...
OLYMPUS_ADMIN_KEY=${OLYMPUS_ADMIN_KEY}
