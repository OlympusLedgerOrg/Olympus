...
|---|---|---|---|
| V3-O1 | Low | **Resolved (2026-05-30)** | ADR-0003/0004/0005 are **Accepted** and the operational leaf-hash binds `shard_id` + `parser_id` + `canonical_parser_version` + `model_hash` via the ADR-0005 structured binary prefix (`crates/olympus-crypto/src/smt.rs`). The verifier/vector rollout is **complete**: `verifiers/test_vectors/vectors.json` now ships `ssmf_existence_proof` / `ssmf_nonexistence_proof` corpora carrying the full binding (`shard_id`, `parser_id`, `canonical_parser_version`, `model_hash`), and **both** offline verifiers reproduce the structured-prefix leaf and pass against it — Rust (`verifiers/rust`, `cargo test`: SSMF existence/non-existence + SMT inclusion/non-inclusion negatives) and JavaScript (`verifiers/javascript`, `test_conformance.js`: `ssmf_existence_proof` 3, `ssmf_nonexistence_proof` 3, ssmf negatives 12). The residual `OLY:LEAF:V1` tag is retained only for the legacy `merkle_leaf_hash` path (a pinned legacy marker per CLAUDE.md), **not** the production SSMF leaf. |
