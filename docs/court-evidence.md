...
|---|---|---|
| **RFC 3161 TSA** | Submission, strict message-imprint + nonce checks, CMS signature verification, and X.509 chain validation with the timestamp-signing certificate purpose. Public TSAs use system roots; private TSAs can add `OLYMPUS_ANCHOR_RFC3161_CA_FILE`. Invalid receipts are not stored. | `openssl ts -verify` against the TSA cert chain remains the independent expert-witness cross-check. |
