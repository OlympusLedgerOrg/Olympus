...
> tools in the right column. Confirm your node includes all four of
> #1058, #1061, #1160, and #1165 before relying on the online column or
> the `metadata.*` checklist at the end of this section.
...
- [ ] Rekor rows show `metadata.set_verified=true`. For the public-good log this needs no configuration — the key comes from the vendored Sigstore trust root. `OLYMPUS_ANCHOR_REKOR_PUBKEY_PEM` is required only for a private Rekor instance, which by definition is not in that root.
...
#    metadata.set_verified=true asserts the SET was checked against the
#    log key resolved from the vendored Sigstore trust root (or from
#    OLYMPUS_ANCHOR_REKOR_PUBKEY_PEM, if that override was set)
