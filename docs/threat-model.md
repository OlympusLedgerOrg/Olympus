...
- **Content confidentiality** — Olympus does not encrypt documents.  Access
  controls are a separate concern. Canonicalization receipts reveal lengths and
  deterministic BLAKE3 commitments; those commitments are binding, not hiding,
  so predictable source content remains vulnerable to offline guessing.
...
| Canonicalization | Deterministic byte sequence from any semantically equivalent input. For the combined ZK claim, a fixed-image RISC Zero guest runs this exact Rust implementation and authenticates a one-section journal that binds the private source commitment to the canonical digest. | [`crates/olympus-crypto/src/canonical.rs`](../crates/olympus-crypto/src/canonical.rs) — `canonicalize_bytes()`, `canonicalize_str()`; [`crates/olympus-crypto/src/canonical_proof.rs`](../crates/olympus-crypto/src/canonical_proof.rs); [`src-tauri/src/zk/canonicalization.rs`](../src-tauri/src/zk/canonicalization.rs); [`docs/adr/ADR-0040-risc-zero-canonicalization-receipts.md`](adr/ADR-0040-risc-zero-canonicalization-receipts.md) |
...
| Narrow-scope mutations only | DB schema does not allow arbitrary updates: <ul><li>`ingest_records` permits only one-shot attach of `zk_bundle` and bounded snapshot-column writes.</li><li>`peer_checkpoints` is mutated only to flag equivocation; peer removal is a soft delete and the checkpoint FK is `ON DELETE RESTRICT`, preserving evidence.</li><li>An OTS row has one monotonic `pending` → `upgraded` transition. The merged receipt must contain a structurally reachable Bitcoin attestation, but metadata remains `bitcoin_attestation_verified=false` and `verified_at=NULL` pending independent chain verification.</li><li>v2 `own_checkpoints` are INSERT-only; migration 0053 puts all newly emitted states under a timestamp-independent partial unique index while preserving historical duplicate rows as evidence.</li></ul>There is no application path that overwrites the cryptographic identity of a checkpoint after insert. | [`migrations/0053_adversarial_audit_hardening.sql`](../migrations/0053_adversarial_audit_hardening.sql); [`src-tauri/src/anchoring/own_checkpoint.rs`](../src-tauri/src/anchoring/own_checkpoint.rs); [`src-tauri/src/anchoring/store.rs`](../src-tauri/src/anchoring/store.rs); [`src-tauri/src/federation/equivocation.rs`](../src-tauri/src/federation/equivocation.rs); [`docs/court-evidence.md` §6](court-evidence.md#6-chain-of-custody--typical-operator-practice) |
| SMT root consistency across federation peers | Wire v3 labels every checkpoint as scope=`shard` with an authenticated `shard_id`; it never advertises one shard snapshot as a global ledger root. The checkpoint signature binds scope, shard, root, height, timestamp, and authority identity. A separately signed Poseidon path proves the final leaf changes the immediately preceding tree root into the current root. The receiver verifies that append witness, binds Groth16 public signals to `ledger_root` and `tree_size`, deduplicates an already-verified statement before pairing work, and keys equivocation/evidence on canonical BJJ identity rather than replaceable peer UUID. | [`src-tauri/src/anchoring/own_checkpoint.rs`](../src-tauri/src/anchoring/own_checkpoint.rs); [`src-tauri/src/federation/verify.rs`](../src-tauri/src/federation/verify.rs); [`src-tauri/src/federation/equivocation.rs`](../src-tauri/src/federation/equivocation.rs) |
...
| Federation peer signature verification with signal binding | Peers require wire v3 and canonical encodings, verify the complete scoped BJJ statement and signed append-consistency witness against the pinned key, require its authority hash to match that key, and bind Groth16 public signals to root/size. Exact authenticated statements already stored with `verified=true` bypass repeat Groth16 work. Equivocation locks, indexes, and blocking use canonical `(BJJ x, BJJ y, scope, shard)` identity, so a replacement UUID cannot hide a fork. | [`src-tauri/src/federation/verify.rs`](../src-tauri/src/federation/verify.rs); [`migrations/0053_adversarial_audit_hardening.sql`](../migrations/0053_adversarial_audit_hardening.sql) |
| Canonicalization-to-inclusion composition | `unified_canonicalization_inclusion_root` first verifies a RISC Zero receipt against the fixed Olympus guest image, validates the one-section journal and expected source commitment, derives the structured section commitment, and requires it to equal Groth16 public signal zero. The raw `unified_section_commitment_inclusion_root` circuit does not claim canonicalization, and the historical `_sign` identifier remains retired. | [`src-tauri/src/api/zk/mod.rs`](../src-tauri/src/api/zk/mod.rs); [`src-tauri/src/zk/canonicalization.rs`](../src-tauri/src/zk/canonicalization.rs); [`docs/adr/ADR-0040-risc-zero-canonicalization-receipts.md`](adr/ADR-0040-risc-zero-canonicalization-receipts.md) |
| Ed25519 signing on the Rekor anchor path | Two distinct guarantees, deliberately not conflated. **Log authenticity:** the signed entry timestamp is verified with ECDSA P-256 at submission against the log key resolved from the vendored Sigstore trust root by the entry's own `logID`, validated at its `integratedTime` (`OLYMPUS_ANCHOR_REKOR_PUBKEY_PEM` overrides for a private Rekor) — this proves *Rekor* countersigned the entry, and nothing about who submitted it (audit M-A2). **Operator attribution:** the entry payload is additionally signed with the operator's Ed25519 key, but the SET check does not inspect that signature, and Olympus ships no binding from that public key to an operator identity. Attribution therefore requires a verifier to check the embedded Ed25519 signature independently *and* to obtain the operator's public key through a trusted channel. | [`src-tauri/src/anchoring/rekor.rs`](../src-tauri/src/anchoring/rekor.rs) |
| RFC 3161 timestamp token | Domain-separated BLAKE3 checkpoint digest submitted to the TSA. Receipt acceptance requires the exact nonce and `TSTInfo.messageImprint`, a valid CMS signature, an ESS signer-certificate binding (including `issuerSerial` when present), and a signer chain trusted for the time-stamping purpose at token `genTime`. Windows requires operator-reviewed roots from `OLYMPUS_ANCHOR_RFC3161_TRUST_ROOTS` and never promotes certificates from the current-user ROOT store; Unix also uses its system trust paths. SHA-256 OID `AlgorithmIdentifier` parameters must be absent or canonical DER NULL (`05 00`); invalid receipts are refused before storage. | [`src-tauri/src/anchoring/rfc3161.rs`](../src-tauri/src/anchoring/rfc3161.rs); [`src-tauri/src/anchoring/tstinfo.rs`](../src-tauri/src/anchoring/tstinfo.rs) |
...
| Anchor cron actually runs (PR #1165) | Before PR #1165 the anchor cron read a never-written BLAKE3 column from `ingest_records` and was silently inert — the `anchor_receipts` table stayed empty under any operator configuration. PR #1165 introduced `own_checkpoints` as the canonical producer; the cron now ticks once per `OLYMPUS_ANCHOR_INTERVAL_SECS`, builds an existence proof + BJJ signature + Ed25519 signature, inserts a row, then calls the RFC 3161 / Rekor / OTS backends for each configured URL. Migration 0052 suppresses known duplicates by `(kind, hash, target)` and provides at-least-once retry semantics, with a lease before network I/O and bounded retry after failure. If a remote accepts a submission but the local process cannot durably record that outcome, a later retry may re-submit it. **Anchoring is opt-in per backend:** zero URLs configured = producer-only mode (rows in `own_checkpoints`, none in `anchor_receipts`). | [`src-tauri/src/anchoring/cron.rs`](../src-tauri/src/anchoring/cron.rs); [`src-tauri/src/anchoring/own_checkpoint.rs`](../src-tauri/src/anchoring/own_checkpoint.rs); [`src-tauri/src/anchoring/store.rs`](../src-tauri/src/anchoring/store.rs); [`migrations/0052_harden_anchor_retry_state.sql`](../migrations/0052_harden_anchor_retry_state.sql); [`docs/court-evidence.md` §1.1](court-evidence.md#11-what-the-v09-binary-actually-enforces-online) |
| Independent court-side verification | The standalone Rust verifier checks Groth16 proofs (`olympus-verifier verify`) and repeats the complete fixed-image RISC Zero receipt, journal, source-commitment, public-signal, H-2, and Groth16 composition (`olympus-verifier verify-canonicalization`). It retains strict snarkjs parsing, curve/subgroup checks, canonical field parsing, vkey fingerprinting, and public-signal bounds. JS `verify-checkpoint --bundle` runs the signature-layer checks and delegates proof verification to Rust. See [`docs/court-evidence.md` §3](court-evidence.md#3-independent-verification--minimal-commands) for the operator runbook. | [`verifiers/rust/src/canonicalization.rs`](../verifiers/rust/src/canonicalization.rs); [`verifiers/rust/src/groth16.rs`](../verifiers/rust/src/groth16.rs); [`verifiers/rust/src/bin/verify.rs`](../verifiers/rust/src/bin/verify.rs); [`verifiers/javascript/verify.js`](../verifiers/javascript/verify.js) |
...
The **combined** `unified_canonicalization_inclusion_root` protocol supplies the
missing content-to-inclusion link. Its fixed-image RISC Zero receipt binds the
private source commitment to the exact Rust canonicalizer's digest; the host
maps that authenticated digest to `canonicalHash`; and the existing Groth16
public inputs (`canonicalHash`, `merkleRoot`, `ledgerRoot`, `treeSize`) carry the
commitment through the Poseidon inclusion paths. The SMT leaf hash itself does
not include the compatibility `poseidon_root` column, so verifiers must perform
the complete receipt-plus-Groth16 composition rather than treating either layer
as sufficient alone.
...
Because there would be no link between the canonical source and the Poseidon
root, the verifier would have no way to detect that the ZK proof describes a
different document than the one named by the source commitment. The composite
protocol closes this gap by binding the receipt-derived canonical digest to the
Groth16 section commitment before accepting its inclusion paths.
...
3. **Canonicalization receipt + Groth16 verification** — Verify the receipt
   against the fixed Olympus image, match its source commitment, derive the
   one-section `canonicalHash`, and require that value as Groth16 public signal
   zero. Then verify the Groth16 inclusion paths through the Poseidon Merkle root
   and ledger root.
...
| Canonicalization zkVM exhaustion | Source and canonical output are each capped at 1 MiB, every execution is capped at 2^30 user cycles, and local proving uses a single-permit queue. The committed eight-shape cycle report peaks at 744,183,696 user cycles, leaving roughly 30.7% of the enforced ceiling unused. | [`crates/olympus-crypto/src/canonical_proof.rs`](../crates/olympus-crypto/src/canonical_proof.rs); [`src-tauri/src/zk/canonicalization.rs`](../src-tauri/src/zk/canonicalization.rs); [`src-tauri/src/api/zk/mod.rs`](../src-tauri/src/api/zk/mod.rs); [`proofs/zkvm/canonicalization/cycle-report.json`](../proofs/zkvm/canonicalization/cycle-report.json) |
