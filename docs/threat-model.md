...
| Ed25519 signing on the Rekor anchor path | The Rekor transparency-log entry payload is signed with the operator's Ed25519 key so the log entry itself is attributable. Sigstore Rekor SET ECDSA-P-256 verification at submission, against the log key resolved from the vendored Sigstore trust root by the entry's own `logID` and validated at its `integratedTime`; `OLYMPUS_ANCHOR_REKOR_PUBKEY_PEM` overrides that for a private Rekor (audit M-A2). | [`src-tauri/src/anchoring/rekor.rs`](../src-tauri/src/anchoring/rekor.rs) |
