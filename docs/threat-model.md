...
| Domain separation | Leaf / node / ledger prefixes prevent cross-context collisions | [`crates/olympus-crypto/src/lib.rs`](../crates/olympus-crypto/src/lib.rs) — `LEAF_PREFIX`, `NODE_PREFIX`, `EMPTY_LEAF_PREFIX`; [`src-tauri/src/anchoring/mod.rs`](../src-tauri/src/anchoring/mod.rs) — checkpoint anchor domain; [`src-tauri/src/api/middleware/auth.rs`](../src-tauri/src/api/middleware/auth.rs) — API-key derivation domain |
