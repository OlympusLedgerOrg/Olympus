...
| OpenTimestamps upgrade | A bounded Timestamp AST parser derives the pending commitment from the original anchored hash, parses the calendar response as a commitment-rooted subtree, requires a Bitcoin attestation, merges it into the original tree, and reparses the persisted bytes. The node records the claimed height/root but deliberately leaves `bitcoin_attestation_verified=false` and `verified_at=NULL`; a tag supplied by a calendar is not Bitcoin consensus verification. | [`src-tauri/src/anchoring/ots_tree.rs`](../src-tauri/src/anchoring/ots_tree.rs); [`src-tauri/src/anchoring/ots.rs`](../src-tauri/src/anchoring/ots.rs); [`src-tauri/src/anchoring/store.rs`](../src-tauri/src/anchoring/store.rs) |
