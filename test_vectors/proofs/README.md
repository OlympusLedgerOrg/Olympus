...
claimed. The recomputed legs are split by implementation: `input_record` →
`canonicalized_bytes_hex` is re-derived by the live encoder in
`crates/olympus-crypto/tests/vector_conformance.rs`; `merkle.*` and `proof.*`
are re-derived by the live verifier in
`verifiers/rust/tests/vector_conformance.rs`, and the file as a whole is
BLAKE3-pinned there so any edit fails CI.
