...
- **Zero-knowledge proofs.** Three production Groth16 circuits cover document existence, non-existence, and a structured-section commitment linked through Merkle and SMT inclusion. The raw unified circuit proves neither canonicalization nor a signature. The public canonicalization/inclusion protocol composes it with a fixed-image RISC Zero receipt that executes the shared Rust canonicalizer. These paths are callable from the desktop binary through scope-gated `/zk/prove` and `/zk/verify` routes (the `prover` feature is on by default; local RISC Zero proving additionally requires supported Linux tooling). The feature-gated federation quorum circuit is also implemented for next-phase quorum policy work. Redaction is no longer a Groth16 circuit; it is verified through signed Merkle replay in the Rust/JavaScript verifier path. **The current Groth16 proving/verification keys are single-contributor development artifacts** (Hermez Phase 1 powers-of-tau, power 20, generated 2026-06-08). A multi-contributor Phase 2 ceremony is the remaining blocker before these proofs should be treated as production-grade. This is stated plainly because it is the most important caveat in the project.
