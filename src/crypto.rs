...
/// Compute a domain-separated leaf hash with parser-version binding.
///
/// Formula:
/// ```text
...
pub(crate) fn compute_leaf_hash(
    ...
    let pid_len = (parser_id.len() as u32).to_be_bytes();
    let cpv_len = (canonical_parser_version.len() as u32).to_be_bytes();
    ...
    hasher.update(parser_id);
    hasher.update(SEP);
    hasher.update(&cpv_len);
    hasher.update(canonical_parser_version);
...
/// Compute a domain-separated hash for a sparse-tree leaf with parser-version binding.
///
/// Formula:
/// ```text
/// BLAKE3(LEAF_PREFIX || SEP || key || SEP || value_hash || SEP ||
...
/// ```
///
/// Both `parser_id` and `canonical_parser_version` must be non-empty strings.
///
/// # Python signature
