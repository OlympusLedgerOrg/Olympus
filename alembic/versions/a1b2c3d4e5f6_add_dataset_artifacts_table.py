...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "timestamp_status", sa.String(length=16), nullable=False, server_default="pending"
        ),
        # External anchor (D6)
        ...
        sa.Column(
            ...
            sa.String(length=32),
            nullable=False,
            server_default="dataset_manifest_v1",
        ),
        sa.Column(
            "canonicalization_method",
            sa.String(length=32),
            nullable=False,
            server_default="canonical_json_v2",
        ),
        ...
        sa.UniqueConstraint(
            "dataset_id", "parent_commit_id", "manifest_hash", name="uq_dataset_commit_content"
        ),
    )
    op.create_index("ix_dataset_artifacts_dataset_id", "dataset_artifacts", ["dataset_id"])
    op.create_index(
        "ix_dataset_artifacts_commit_id", "dataset_artifacts", ["commit_id"], unique=True
    )
    ...
    op.create_index(
        "ix_dataset_artifacts_committer_pubkey", "dataset_artifacts", ["committer_pubkey"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "artifact_id",
            sa.String(length=36),
            sa.ForeignKey("dataset_artifacts.id"),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_dataset_artifact_files_artifact_id", "dataset_artifact_files", ["artifact_id"]
    )
    ...
    op.create_table(
        ...
        # Replay protection
        sa.UniqueConstraint(
            "dataset_id", "model_id", "event_type", "committer_pubkey", name="uq_lineage_event"
        ),
    )
    op.create_index(
        "ix_dataset_lineage_events_dataset_id", "dataset_lineage_events", ["dataset_id"]
    )
    op.create_index("ix_dataset_lineage_events_model_id", "dataset_lineage_events", ["model_id"])
    op.create_index(
        "ix_dataset_lineage_events_commit_id", "dataset_lineage_events", ["commit_id"], unique=True
    )
