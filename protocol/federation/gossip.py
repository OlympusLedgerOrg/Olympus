...
def build_proactive_share_commitments(
    ...
    for node in registry.active_nodes():
        payload = encode_signing_fields(node.node_id, node.pubkey.hex(), str(epoch), refresh_nonce)
        commitments[node.node_id] = blake3_hash(
            [PROACTIVE_SHARE_COMMIT_PREFIX, b"|", payload]
        ).hex()
    return commitments
...
def build_vrf_reveal_commitment(*, node_id: str, reveal: str) -> str:
    ...
    return blake3_hash([_VRF_COMMIT_REVEAL_PREFIX, b"|", payload]).hex()
...
def derive_vrf_round_entropy(
    *,
    ...
    # Wrap context + all per-node chunks in a single length-prefixed payload so
    # field boundaries are unambiguous across the entire input.
