...
def test_event_id_prevents_field_injection() -> None:
    ...
    def legacy_separator_event_id(shard_id: str, header_hash: str, timestamp_value: str) -> str:
        event_data = HASH_SEPARATOR.join([shard_id, header_hash, timestamp_value])
        return blake3_hash([EVENT_PREFIX, _SEP, event_data.encode("utf-8")]).hex()

    colliding_legacy = legacy_separator_event_id("X|Y", "Z", timestamp)
    assert colliding_legacy == legacy_separator_event_id("X", "Y|Z", timestamp)
