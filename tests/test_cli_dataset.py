...
def bundle_file(tmp_path, key_prefix, dataset_dir):
    ...
    result = subprocess.run(
        [
            ...
            "--source-uri", "https://example.com/dataset",
...
def test_commit_stdout_is_valid_json(key_prefix, dataset_dir):
    ...
    for field in ("dataset_id", "commit_id", "parent_id", "manifest", "committer_pubkey", "signature"):
...
def test_commit_root_hash_stable_for_duplicate_content(key_prefix, tmp_path):
    ...
    assert paths == ["a.txt", "B.txt"], (
        f"Expected casefold order ['a.txt', 'B.txt'], got {paths}"
    )
...
def test_commit_missing_private_key(dataset_dir, tmp_path):
    result = subprocess.run(
        [
            ...
            "commit",
            str(dataset_dir),
            "--private-key", str(tmp_path / "no.priv"),
            "--dataset-name", "x",
            "--source-uri", "https://example.com",
            "--namespace", "ns",
        ],
...
def test_olympus_dataset_commit_and_verify_round_trip(tmp_path):
    ...
    subprocess.run(
        [
            ...
            "commit",
            str(ds),
            "--private-key", f"{prefix}.priv",
            "--dataset-name", "round-trip",
            "--source-uri", "https://example.com",
            "--namespace", "test",
            "-o", str(bundle_path),
        ],
