...
class TestCanonicalJsonParity:
    ...
    def test_string_surrogate_pair_encoding(self) -> None:
        # U+1F600 GRINNING FACE — encoded as \\uD83D\\uDE00 in JSON
...
def test_rust_canonical_golden_vectors() -> None:
    """
    For every positive input vector in canonicalizer_vectors.tsv, verify that
    the Rust ``canonical_json_encode`` produces byte-for-byte identical output
    to the pure-Python ``canonical_json_encode``.

    Note: the TSV's ``canonical_hex`` column was generated by the JCS
    ``protocol.canonicalizer.Canonicalizer`` which emits raw UTF-8 (non-ASCII
    safe).  ``canonical_json_encode`` uses ``ensure_ascii=True`` and will
    produce different but internally self-consistent output.  This test
    therefore validates Rust ↔ Python parity, not Rust ↔ TSV column parity.
    ...
    with _VECTORS_PATH.open(encoding="utf-8") as fh:
        for line in fh:
            ...
            group_id, input_hex, _canonical_hex, _hash_hex = parts
            ...
            try:
                py_output = _py_canonical_json_encode(obj)
            except Exception:
                # If Python rejects it, Rust should too; skip rather than fail
                continue

            # Rust output — must exactly match Python
            try:
                rust_output = _rust_canonical.canonical_json_encode(obj)
            ...
            if rust_output != py_output:
                mismatches.append(
                    f"Vector {group_id!r}:\n"
                    f"  python: {py_output!r}\n"
                    f"  rust:   {rust_output!r}"
                )
            total += 1
