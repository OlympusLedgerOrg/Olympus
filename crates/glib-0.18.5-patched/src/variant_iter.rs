...
impl<'a> VariantStrIter<'a> {
    ...
    fn impl_get(&self, i: usize) -> &'a str {
        unsafe {
            // SECURITY (Olympus backport of gtk-rs/gtk-rs-core#1343,
            // GHSA-wrw7-89jp-8q8g): `g_variant_get_child` writes the
            // child string pointer back into `p` via the out-argument,
            // which requires `&mut p`. Upstream 0.18.5 declares `let p`
            // and passes `&p`; the compiler is then free to DCE the
            // unsound write, producing a NULL deref. Fix is 0.20.0
            // (PR #1343); we ship a 2-line backport here so Linux
            // bundles can stay on the GTK-0.18 webkit2gtk chain.
