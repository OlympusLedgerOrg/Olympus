//! BN254 Poseidon for ZK-circuit witness generation and redaction commitments.
//!
//! The permutation parameters (round constants, MDS matrix) match circomlibjs
//! exactly so that outputs are bit-for-bit identical to the Groth16 circuits.
//!
//! This module is only compiled when the `poseidon` Cargo feature is enabled.
...
pub enum PoseidonError {
    #[error("BLAKE3 hex digest must be exactly 64 characters (32 bytes); got {0}")]
    InvalidLength(usize),
    #[error("invalid hex in BLAKE3 digest")]
    InvalidHex,
}
...
// Parameters for t=3 (2 inputs + 1 capacity), nRoundsF=8, nRoundsP=57.
// Source: circomlibjs/src/poseidon_constants.json entry C[1] (t-2=1), M[1].
// These are identical to the constants in src/poseidon.rs in the workspace root.
...
fn poseidon_permutation(state: &mut [Fr; 3]) {
    ...
    for r in 0..(FULL_ROUNDS + PARTIAL_ROUNDS) {
        ...
        let old = *state;
        state[0] = mds[0][0] * old[0] + mds[0][1] * old[1] + mds[0][2] * old[2];
        state[1] = mds[1][0] * old[0] + mds[1][1] * old[1] + mds[1][2] * old[2];
        state[2] = mds[2][0] * old[0] + mds[2][1] * old[1] + mds[2][2] * old[2];
    }
}

/// Plain Poseidon(a, b) → state[0] after permutation.
/// Matches `poseidon_hash_bn128` / `poseidon_hash_bn254` from the Python layer.
...
fn poseidon_with_domain(left: &BigUint, right: &BigUint, domain: u64) -> BigUint {
    ...
    let r = biguint_to_fr(right);
    let tagged_left = poseidon_hash(d, l);
    fr_to_biguint(poseidon_hash(tagged_left, r))
}
...
pub fn blake3_hex_to_poseidon_leaf(hex_hash: &str) -> Result<BigUint, PoseidonError> {
    if hex_hash.len() != 64 {
        return Err(PoseidonError::InvalidLength(hex_hash.len()));
    }
    let raw = hex::decode(hex_hash.to_lowercase()).map_err(|_| PoseidonError::InvalidHex)?;
    let field_elem = blake3_bytes_to_field(&raw);
    let domain = Fr::from(DOMAIN_LEAF);
    let leaf = poseidon_hash(domain, field_elem);
...
pub fn compute_poseidon_commitment_root(leaves: &[BigUint]) -> BigUint {
    ...
    for leaf in &leaves[1..] {
        acc = poseidon_with_domain(&acc, leaf, DOMAIN_COMMITMENT);
    }
    acc
}
...
pub fn compute_redaction_commitments(
    ...
    let seed = BigUint::from(revealed_count);
    let mut acc = poseidon_with_domain(&seed, &revealed_leaves[0], DOMAIN_COMMITMENT);
...
mod tests {
    ...
    fn poseidon_hash_is_deterministic() {
        ...
        let r1 = poseidon_with_domain(&a, &b, DOMAIN_COMMITMENT);
        let r2 = poseidon_with_domain(&a, &b, DOMAIN_COMMITMENT);
        assert_eq!(r1, r2);
    }
    ...
    fn poseidon_domain_separates() {
        ...
        let r1 = poseidon_with_domain(&a, &b, DOMAIN_COMMITMENT);
        let r2 = poseidon_with_domain(&a, &b, DOMAIN_MASK);
        assert_ne!(r1, r2, "different domains must produce different hashes");
    }

    #[test]
    fn blake3_hex_to_leaf_rejects_short_input() {
        let result = blake3_hex_to_poseidon_leaf("aabbcc");
        assert!(result.is_err());
    }

    #[test]
    fn blake3_hex_to_leaf_rejects_non_hex() {
        let bad = "z".repeat(64);
        let result = blake3_hex_to_poseidon_leaf(&bad);
        assert!(result.is_err());
    }

    #[test]
    fn blake3_hex_to_leaf_valid_zero_hash() {
        let hex = "0".repeat(64);
        let result = blake3_hex_to_poseidon_leaf(&hex);
        assert!(result.is_ok());
    ...
    #[test]
    fn commitment_root_seeded_by_length() {
        let leaves: Vec<BigUint> = (0..4).map(|i| BigUint::from(i as u64 + 1)).collect();
        let root = compute_poseidon_commitment_root(&leaves);
