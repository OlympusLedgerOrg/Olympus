//! Binary Merkle tree (domain-prefixed BLAKE3) + a Poseidon/BN254 variant.
//!
//! Relocated out of the desktop crate so it can be unit-tested without the GUI
//! toolchain and shared with the offline verifiers. Pure crypto: depends only on
//! BLAKE3, serde, and this crate's `poseidon` module.
//!
//! Tree shape follows RFC 6962 (Certificate Transparency): pairs are hashed
//! `(left, right)`, and a lone trailing node at an odd level is **promoted** to
//! the next level unchanged — never duplicated. This matches the reference
//! verifier in `verifiers/rust` (`compute_merkle_root`) for any leaf count and
//! removes the "phantom index" forgery class that duplicate-padding allowed.
//!
//! `verify_proof` derives the walk direction and promotion points from
...
fn hex_to_fr(hex: &str) -> Result<Fr, PoseidonError> {
    let decoded = hex::decode(hex).map_err(|_| PoseidonError::InvalidHex)?;
    if decoded.len() > 32 {
        return Err(PoseidonError::InvalidLength(decoded.len()));
    }
...
fn poseidon_leaf(content_hash: &str) -> Result<Fr, PoseidonError> {
    let val = hex_to_fr(content_hash)?;
    Ok(poseidon::domain_node(2, Fr::from(0u64), val))
}

/// Domain 1 node: DomainPoseidon(1, left, right)
fn poseidon_node(left: Fr, right: Fr) -> Fr {
...
/// hash fails to decode — never a tree built on silently-dropped leaves.
pub fn build_poseidon_tree(content_hashes: &[String], target_hash: &str) -> Option<PoseidonResult> {
    ...
    while level.len() > 1 {
        ...
        for i in (0..level.len()).step_by(2) {
            if i + 1 < level.len() {
                next.push(poseidon_node(level[i], level[i + 1]));
            } else {
...
mod tests {
    ...
    fn poseidon_multi_leaf() {
        ...
        // All should produce the same root.
