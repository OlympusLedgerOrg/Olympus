...
const CredentialsPage: React.FC = () => {
  ...
  const issue = async () => {
    ...
    if (newDetails.trim()) {
      try {
        parsed = JSON.parse(newDetails);
      } catch (e) {
        setError(`details JSON: ${(e as Error).message}`);
        return;
      }
    }
    setIssuing(true);
  ...
  const revoke = async (id: string) => {
    if (
      !window.confirm(
        `Revoke credential ${id}?\n\nIrreversible. A revocation signature will be written and the credential will be flagged invalid for verifiers.`,
      )
    )
      return;
    try {
    ...
    try {
      ...
      window.alert(
        `commit_id matches:      ${r.commit_id_matches}\n` +
          ...
          (r.is_revoked ? `revoked sig valid:      ${r.revoked_signature_valid ?? "n/a"}\n` : "") +
          (r.commitment_opens != null ? `commitment opens:       ${r.commitment_opens}\n` : "") +
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: 1100, margin: "0 auto" }}>
      ...
      >
        Olympus-native Soulbound Tokens — non-transferable credentials cryptographically signed by
        the federation's BJJ authority key. Verifiable offline by anyone holding the federation
        pubkey; no blockchain involved.
      </p>
      ...
      {justIssued && (
        ...
        >
          ...
          <div style={{ fontSize: "0.6rem", color: "rgba(0,255,65,0.6)", marginBottom: "0.4rem" }}>
            id: <code>{justIssued.id}</code> · commit:{" "}
            <code>{justIssued.commit_id.slice(0, 24)}…</code>
            {justIssued.commitment && (
              <>
                {" "}
                ·{" "}
                <span style={{ color: "#ffcc66" }}>
                  🔒 private (Pedersen v{justIssued.commitment.version})
                </span>
              </>
            )}
          </div>
          {justIssued.opening && (
            ...
            >
              ...
              >
                The server stored only the commitment and discarded the cleartext details. Save this
                opening now — it is the holder's only way to later prove the credential, and it is
                not recoverable.
              </div>
              ...
              <button
                type="button"
                style={{ ...btn("primary"), marginTop: "0.4rem" }}
          ...
          <button
            type="button"
            style={btn("primary")}
            onClick={() => navigator.clipboard.writeText(JSON.stringify(justIssued, null, 2))}
          >
            COPY FULL JSON
          </button>
          <button
            type="button"
            style={{ ...btn("ghost"), marginLeft: "0.4rem" }}
      ...
      >
        ...
        >
          <div>
            ...
            <input
              ...
              placeholder="user:<uuid>, email:..., bjj:<x>:<y>"
              style={inp}
            />
          </div>
          <div>
        ...
        <div>
          <label style={lbl}>
            DETAILS (JSON, optional — hashed into commit; discarded if committing privately)
          </label>
          <textarea
            value={newDetails}
        ...
        >
          ...
          >
            <div>
              ...
              <div
                style={{ fontSize: "0.58rem", color: "rgba(0,255,65,0.5)", marginTop: "0.2rem" }}
              >
                holder: <code>{c.holder_key}</code> · id: <code>{c.id}</code>
              </div>
            </div>
            <div style={{ display: "flex", gap: "0.4rem" }}>
              <button type="button" style={btn("ghost")} onClick={() => verify(c)}>
                VERIFY
              </button>
              <button
                type="button"
              ...
              {!c.revoked_at && (
                <button type="button" style={btn("danger")} onClick={() => revoke(c.id)}>
                  REVOKE
                </button>
              )}
            </div>
          ...
          >
            ...
            {c.commitment && (
              <>
                <br />
                <span style={{ color: "#ffcc66" }}>
                  🔒 private commitment (Pedersen v{c.commitment.version}) · x:
                  <code>{c.commitment.x.slice(0, 16)}…</code> y:
                  <code>{c.commitment.y.slice(0, 16)}…</code>
                </span>
              </>
            )}
          </div>
